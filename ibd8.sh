...
f=$(sed -n "${SGE_TASK_ID}{p;q;}" 2018_tar.txt)
echo "THIS RUN IS--------------------------------- $f --------------"
DATA="/home3/redwards/IBD/HMP2/MGX/2018-05-04"
mkdir temp_$f
newf=$(echo $f | sed -e s/.fastq//g)
fastq=~/cpg/temp_$f/$newf.fastq
outtemp=~/cpg/temp_$f

## Unzip the file to a new temp directory
tar -xf /home3/redwards/IBD/HMP2/MGX/2018-05-04/$f -C $outtemp/ 
gunzip -c $outtemp/* >> $fastq
...
~/bin/Python-3.6.0/python IBD_Kmer_Analysis/count_motifs.py -i $fastq -o results/$f.motifs.csv
